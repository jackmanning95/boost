/*
  # Company & User Management Migration

  1. Database Structure Updates
    - Add account_id column to companies table
    - Update user role constraints to support new roles
    - Ensure proper foreign key relationships

  2. Role Management
    - Support for super_admin, admin, and user roles
    - Auto-assign company admin to first user in company
    - Super admin detection based on @boostdata.io email domain
...
-- Step 1: Only disable user-defined triggers (not system triggers)
DO $$
DECLARE
    trigger_record RECORD;
BEGIN
    -- Disable only user-defined triggers on users table
...
-- Step 10: Re-enable user-defined triggers and update trigger
DO $$
DECLARE
    trigger_record RECORD;
BEGIN
    -- Re-enable only user-defined triggers on users table
    FOR trigger_record IN 
        SELECT tgname 
        FROM pg_trigger 
        WHERE tgrelid = 'users'::regclass 

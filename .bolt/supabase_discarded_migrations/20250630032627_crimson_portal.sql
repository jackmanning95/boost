...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'get_user_company_id function already exists';
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'is_user_company_admin' 
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) THEN
    ...
    RAISE NOTICE 'Created is_user_company_admin function';
  ELSE
    RAISE NOTICE 'is_user_company_admin function already exists';
  END IF;
END $$;

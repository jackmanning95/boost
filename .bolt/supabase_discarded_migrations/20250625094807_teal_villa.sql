/*
  # Debug Company Account Permissions Function
  
  This function helps debug why Platform IDs aren't loading by checking:
  1. Current user authentication status
  2. User's company association
  3. Available company account IDs
...
BEGIN
  ...
  -- Get company account IDs (bypassing RLS for debugging)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', ca.id,
...
-- Add comment
COMMENT ON FUNCTION debug_company_account_permissions() IS 'Debug function to troubleshoot company account ID permissions and RLS policies';
/*
  ...
  1. Database Schema Updates
    ...
    - Update user roles and constraints
    - Create helper functions for role management
    
  2. Security
    - Enhanced RLS policies for company-based access
    - Super admin detection and privileges
    - Company admin role management
    
  3. Triggers
    - Auto-assign company admin to first user
    - Super admin detection based on email domain
*/

-- Step 1: Update existing data BEFORE applying constraints
UPDATE users SET role = 'user' WHERE role = 'client';

-- Step 2: Add account_id column to companies table
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 3: Update users table constraints (drop old constraint first)
DO $$
BEGIN
...
-- Step 5: Ensure company_id foreign key exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
...
-- Step 7: Create trigger function for new user registration
...
-- Step 10: Create new Companies policies
...
-- Step 11: Drop existing Users policies
DROP POLICY IF EXISTS "Users can read own profile" ON users;
DROP POLICY IF EXISTS "Users can update own profile" ON users;

-- Step 12: Create new Users policies
...
-- Step 13: Drop existing Campaign policies
...
-- Step 14: Create new Campaign policies
...
-- Step 15: Drop existing Audience Request policies
...
-- Step 16: Create new Audience Request policies
...
-- Step 19: Create performance indexes
...
-- Step 20: Update existing super admin users (do this LAST after constraint is applied)
UPDATE users 

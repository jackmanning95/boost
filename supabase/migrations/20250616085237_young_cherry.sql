/*
  ...
  1. New Features
    - Add account_id column to companies table
    - Update user roles to support super_admin, admin, user
    - Create company hierarchy and access controls
    - Enhanced RLS policies for multi-tenant access

  2. Security
    - Super admins (@boostdata.io emails) can access everything
    - Company admins can manage their company users
    - Users can only access their own company data
    - Proper RLS policies for all tables

  3. Changes
    - Update role constraint from 'client' to 'user'
    - Add company admin detection functions
    - Create trigger for auto-assigning first user as company admin
    - Enhanced policies for campaigns, requests, notifications
*/

-- Step 1: Add account_id column to companies table
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 2: Create helper functions first (before any data updates)
...
-- Step 3: Drop the existing constraint temporarily
DO $$
BEGIN
  IF EXISTS (
...
-- Step 4: Update existing data (now without constraint blocking us)
UPDATE users SET role = 'user' WHERE role = 'client';
UPDATE users SET role = 'user' WHERE role NOT IN ('admin', 'user', 'super_admin');

-- Step 5: Update super admin users based on email domain
UPDATE users 
...
-- Step 6: Now add the new constraint (after data is clean)
ALTER TABLE users ADD CONSTRAINT valid_role 
  CHECK (role = ANY (ARRAY['admin'::text, 'user'::text, 'super_admin'::text]));

-- Step 7: Ensure company_id foreign key exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
...
-- Step 10: Drop existing policies for Companies
...
-- Step 11: Create new Companies policies
...
-- Step 12: Drop existing Users policies
DROP POLICY IF EXISTS "Users can read own profile" ON users;
DROP POLICY IF EXISTS "Users can update own profile" ON users;

-- Step 13: Create new Users policies
...
-- Step 14: Drop existing Campaign policies
...
-- Step 15: Create new Campaign policies
...
-- Step 16: Drop existing Audience Request policies
...
-- Step 17: Create new Audience Request policies
...
-- Step 20: Create performance indexes

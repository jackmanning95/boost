/*
  ...
  1. New Security Definer Functions
    - `get_user_company_id`: Retrieves a user's company ID without triggering RLS
    - `is_user_company_admin`: Checks if a user is a company admin without triggering RLS

  2. RLS Policies
    - Drop all existing policies on users table
    - Create new non-recursive policies
    - Ensure service_role has full access (critical for invite-user function)
    - Allow users to manage their own profiles
    - Allow super admins full access
    - Allow company members to read other members in their company

...
-- Create companies table for proper company management (only if it doesn't exist)
DO $$
BEGIN
...
-- Add updated_at trigger for companies (drop first if exists)
...
-- Add foreign key constraint for company_id (drop first if exists)
DO $$
BEGIN
...
-- Create campaign activity log table (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'campaign_activity_log') THEN

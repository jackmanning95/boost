/*
  # Fix RLS Policies and Update User Roles

  1. Updates jackmanning70@gmail.com to admin role
  2. Fixes RLS policies to avoid recursion
  3. Ensures advertiser accounts persist properly
  4. Allows all company members to view team list
*/
...
-- Step 9: Add unique constraint to prevent duplicate platform/account_id combinations per company (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE table_name = 'company_account_ids' 
    AND constraint_name = 'unique_company_platform_account'
  ) THEN
...
-- Step 12: Ensure proper foreign key constraints exist
DO $$
BEGIN
